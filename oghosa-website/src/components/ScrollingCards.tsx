...
const ScrollingCards = () => {
  ...
  return (
    <div className="scrolling-cards-container min-w-0 bg-gradient-to-r from-brand-950 via-slate-700 to-slate-500 justify-center shadow-lg rounded-lg p-2 sm:p-4">
      <div ref={containerRef} className="card-container relative flex w-full min-w-0">
        {cardData.map((card, index) => (
          <div
            key={index}
            className="cards flex h-full flex-col items-center justify-center border bg-white p-4 shadow-md sm:p-6 rounded-lg"
          >
      ...
      <div className="actions flex justify-center space-x-4 mt-4">
        <button onClick={scrollToPrev} className="px-4 py-2 bg-brand-600 text-white rounded hover:bg-brand-700">Prev</button>
        <button onClick={scrollToNext} className="px-4 py-2 bg-brand-600 text-white rounded hover:bg-brand-700">Next</button>
      </div>
    </div>
  );
};
